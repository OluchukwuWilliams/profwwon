...
# querry the db means to get data from the db
# persist to the db means to send items in the db
# def index
# def categories(request, slug):
#   categories = product.objecs.filter(categories__slug=slug) 
...
def categories(request, slug):
    categories = Product.objects.filter(category_id=id)
...
def contact(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save
            messages.success(request, f"Dear Your message was sent successfully")
...
def signup(request):
    ...
    if request.method =='POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f"welcome {newuser.username}!, Signup successfully")
            send_mail (
                "Thank Yoy",
...
def displaycart(request):
    ...
    return render(request, 'displaycarts.html', context)

login_required(login_url='signin')
def deleteitem(request):
    if request.method == 'POST':
        items_id = request.POST['item_id']
        deleteitem = Shopcart.objects.get(pk=items_id)
        deleteitem.delete()
...
def pay(request):
    if request.method == 'POST':
        ...
        cburl = 'http://34.205.143.234/callback/'
        ref = str(uuid.uuid4())
        profile = Profile.objects.get(user__username=request.user.username)
